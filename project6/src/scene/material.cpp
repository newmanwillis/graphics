...
Vec3d Material::shade( Scene *scene, const ray& r, const isect& i ) const
{
    ...
    // Diffuse Light
	result += prod(kd(i), pLight->getColor(r.at(i.t))) * max(pLight->getDirection(r.at(i.t)) * i.N, 0.0);

	// Specular Light
    Vec3d halfAngle = pLight->getDirection(r.at(i.t)) + (r.at(0.0) - r.at(i.t));
    halfAngle.normalize();
    float rv = halfAngle * i.N;  // (r.at(0.0) - r.at(i.t));
    result += prod(ks(i), pLight->getColor(r.at(i.t))) * max(pow(rv, shininess(i)), 0.0);    
	}
